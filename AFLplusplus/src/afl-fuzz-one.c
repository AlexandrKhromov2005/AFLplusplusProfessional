...
/* we are through with this queue entry - for this iteration */
  ...
  /* === ML: залогировать результат для обучения === */
  if (afl->ml_sched && afl->ml_cur_energy_assigned > 0) {

    /* queued_with_cov — кол-во entries с новым покрытием, proxy для новых edges */
    u32 new_edges_approx = afl->queued_with_cov;

    ml_features_t feat;
    struct queue_entry *cur_entry = (afl->ml_cur_entry_id < afl->queued_items)
                                    ? afl->queue_buf[afl->ml_cur_entry_id] : NULL;
    if (cur_entry) {
      ml_build_features(afl, cur_entry, &feat);
      ml_log_result(afl->ml_sched,
                    afl->ml_cur_entry_id,
                    afl->ml_cur_energy_assigned,
                    new_edges_approx,
                    0,
                    afl->saved_crashes > 0 ? 1 : 0,
                    afl->saved_hangs > 0 ? 1 : 0,
                    &feat);
    }

    afl->ml_cur_energy_assigned = 0;
