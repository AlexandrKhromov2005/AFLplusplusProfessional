...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  if (afl->ml_sched && afl->ml_sched->daemon_available) {
    ...
    ml_features_t feat;
    ml_build_features(afl, q, &feat);

    int32_t ml_energy = ml_request_energy(afl->ml_sched, &feat);
    ...
    /* Демон не ответил — fallback на стандартный AFL-шедулер */
    afl->ml_sched->afl_total_decisions++;
