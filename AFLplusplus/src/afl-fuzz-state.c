...
void afl_state_init(afl_state_t *afl, uint32_t map_size) {
  ...
  /* === ML Scheduler инициализация === */
  {
    const char *ml_socket = getenv("MLF_SCHEDULER_SOCKET");
    const char *ml_log    = getenv("MLF_TRAINING_LOG");

    if (getenv("MLF_SCHEDULER_DISABLE") == NULL) {
      afl->ml_sched = ml_sched_init(ml_socket, ml_log);
      if (!afl->ml_sched) {
        WARNF("ML Scheduler: failed to initialize (running without ML)");
      }
    } else {
      afl->ml_sched = NULL;
    }
    ...
    afl->ml_cur_new_edges       = 0;
    afl->ml_cur_new_hit_counts  = 0;
    afl->ml_cur_entry_id        = 0;
  }
  /* === конец ML init === */
...
void afl_state_deinit(afl_state_t *afl) {
  ...
  /* === ML Scheduler завершение === */
  if (afl->ml_sched) {
    ml_sched_destroy(afl->ml_sched);
    afl->ml_sched = NULL;
  }
  /* === конец ML deinit === */
