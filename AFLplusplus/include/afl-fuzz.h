...
/* Forward declaration for ML scheduler state (defined in ml-scheduler.h) */
struct ml_sched_state;
...
#define N_FUZZ_SIZE_BITMAP (1 << 29)
  ...
  /* ML Power Scheduler state (NULL если ML отключён) */
  struct ml_sched_state *ml_sched;

  /* Счётчик new_edges для текущего fuzz_one() вызова */
  u32 ml_cur_new_edges;
  u32 ml_cur_new_hit_counts;
  u32 ml_cur_energy_assigned;
  u32 ml_cur_entry_id;
